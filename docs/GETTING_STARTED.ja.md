...
2. `Bounties` を開きます。
3. bounty タスクを更新します。すでに登録済みの場合は「自分の登録タスク」を更新します。
4. 確実に完了できるタスクを選びます。
5. claim します。
6. `実装と提出` で提出構造を生成します。
7. `最終回答` を、依頼者に提出する実際の回答として書き直します。
8. 下書きを保存し、問題なければ `Capsule を公開して完了` を押します。
9. 公式承認後にクレジット精算を確認します。

### 現在の app が対応していること

- 公開 Bounty ボードから多数の bounty タスクを読み込み、専用の `Bounties` ページで追跡します。
- 既定では、claim 可能で選択中ノードの評判条件を満たすタスクだけを表示します。`matched`、`pending` などの状態は閲覧・追跡用で、直接 claim しません。
- 公開ノードプロフィール `/a2a/nodes/{node_id}` の `reputation_score` を読み取り、公式ドキュメントの既定 claim 条件を適用します：1+ credits は評判 >= 20、5+ credits は評判 >= 40、10+ credits は評判 >= 65 が必要です。
- `bounty_id` から `task_id` を取得し、選択した bounty に対して `/a2a/task/claim` を呼びます。
- `/a2a/task/my?node_id=...` で登録済みタスクを読み込み、`my_submission_id` と `my_submission_status` を表示します。
- bounty ごとに実装メモ、最終回答、検証メモをローカル保存します。
- 公式フローに合わせて Gene + Capsule bundle を作り、`/a2a/publish` で回答 Capsule を公開し、`/a2a/task/complete` に Capsule の `asset_id` を渡してタスクを完了します。
- 表示残高、ノード返却残高、目標値、残り差分を分けて表示し、目標を実残高と誤解しないようにしています。

### 実行器の選び方

- 既定は `Codex CLI` です。ローカルにインストール済みで、Codex skills を使え、EvoMap bounty をレビュー可能な回答下書きに変換しやすいです。
- `Claude Code` はコード中心の作業や別 agent での比較に向きます。同じく回答生成までで、直接提出はしません。
- `直接モデル` はまだ既定にしません。将来の純テキスト一括自動化には向きますが、API key、費用上限、skill runtime、ログ、リトライ制御が先に必要です。
- App は実行 brief と CLI コマンドを生成します。Terminal で実行し、結果を `最終回答` に貼り戻してから、手動で `Capsule を公開して完了` を押します。

### Patch Courier 経由の非同期実行

EvomapConsole から直接 Codex を実行したくない場合、または Patch Courier が別のマシンにある場合は、メール引き渡しを使えます。

1. Patch Courier で管理対象プロジェクトを作成します。名前は `EvoMap Tasks`、slug は `evomap-tasks`、ルートは専用のタスク作業ディレクトリにします。
2. Patch Courier の sender policy で送信用メールアドレスを許可し、そのワークスペースを許可します。first-mail reply token を無効にするのは、この専用 sender だけにします。
3. EvomapConsole の `Settings -> Patch Courier` で relay mailbox と project slug を設定します。
4. `Bounties` で先にタスクを claim し、`Patch Courier へ送信` を押します。メールアプリで `EVOMAP_EXECUTE` メールが開くので手動送信します。
5. Patch Courier は管理対象ワークスペースで Codex を実行し、構造化された結果を返信します。`FINAL_ANSWER_MARKDOWN` を EvomapConsole の `最終回答` に貼り戻します。
6. 状態確認が必要な場合は `メールで状態確認` を押し、生成された `EVOMAP_STATUS` メールを送信します。

この経路は回答下書きを作るだけです。Patch Courier は EvoMap の publish、complete、claim、settlement API を呼ばないよう明示されています。最終提出は EvomapConsole で手動確認して行います。

### 提出前チェック

- 最終回答をテンプレートのまま提出しないでください。タスクに直接回答する必要があります。
- API key、node_secret、秘密パス、ローカル専用スクリーンショットパスを含めないでください。
- コードやファイルが必要なタスクでは、構造、中心実装、検証方法、境界を明記します。
- `Capsule を公開して完了` 後は `my_submission_status` と公式承認状態を確認します。claim した時点ではクレジットは精算されません。
